...
class Station(object):
    ...
    def station_update(self, current_time, stations, passenger_update_interval=1):
        ...
        if self.od is not None:  # 确保存在OD矩阵
            hour_offset = int(int(current_time) // 3600)          # avoid float-div '12.0:00:00'
            effective_hour = max(6, min(6 + hour_offset, 19))     # clamp to [06,19] matching OD
            effective_period_str = f"{effective_hour:02d}:00:00"  # e.g. '12:00:00'
